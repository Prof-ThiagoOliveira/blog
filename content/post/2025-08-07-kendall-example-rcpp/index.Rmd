...
Let $\{(x_i,y_i)\}_{i=1}^n$ be paired observations. For each unordered pair $(i,j)$ with $i<j$. Now we can define the following:

-   **Concordant** if $(x_i-x_j)(y_i-y_j)>0$\
-   **Discordant** if $(x_i-x_j)(y_i-y_j)<0$\
-   **Tie in** $x$ if $x_i=x_j$ (irrespective of $y$)\
-   **Tie in** $y$ if $y_i=y_j$ (irrespective of $x$)

Define the counts

$$
C = \sum_{1\le i<j\le n} \mathbf{1}\!\left((x_i - x_j)(y_i - y_j) > 0\right)
$$

$$
D = \sum_{1\le i<j\le n} \mathbf{1}\!\left((x_i - x_j)(y_i - y_j) < 0\right), \text{ and}
$$
...
Let the $x$-values contain tie groups of sizes $\{t_{x,g}\}$ and the $y$-values tie groups $\{t_{y,h}\}$. The numbers of tied pairs are
...
Appropriate when ties are absent or negligible.
...
Adjusts the normalisation for ties in $x$ and $y$, preserving the $[-1,1]$ range. When there are no ties ($T_x=T_y=0$), consequently, $\tau_b=\tau_a$.

> **Convention.** Pairs tied on both variables are included in both $T_x$ and $T_y$, matching common software implementations of $\tau_b$.

# Rcpp inline implementation (tau-a)

The `C++` function below computes Kendall’s **tau-a** (no tie correction). For practical datasets with ties, tau-b is usually preferable; see the caveats.
