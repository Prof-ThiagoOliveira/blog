...
tags:
  ...
  - C++
  - Algorithm
subtitle: 'Plan first and then   translate everything as code'
summary: ""
featured: no
image:
  caption: ''
...
Testing your algorithm is a useful step to ensure steps 1-3 are actually right before we proceed to the step 5. Some examples about what you should do during this stage are described below:
...
A good example of algorithm mistake could be seen in the Example 2. What happens if $y=0$, or $y<0$? We can see that our algorithm handles these cases incorrectly. If you calculate the algorithm steps by hand with $x=2$ and $y=0$ you will get $2^0=2$ rather than $2^0=1$ (correct answer). Additionally, for any value $y\leq 0$ the algorithm try to count from $1$ to $y-1<0$, of which are no Natural number, leading an error in the process. Thus, we can conclude that $|y| \in \mathcal{N}$, where $\mathcal{N}$ represents Natural numbers. In this sense, we might attempt to generalize our algorithm to higher number of cases:
```
Algorthm 2 
...
<div class="div-3">
Question: How can we improve this algorithm?
</div>
