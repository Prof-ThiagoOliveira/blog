...
- [1) Why SQL matters in agriculture analytics](#1-why-sql-matters-in-agriculture-analytics)
- [2) What is a table, row, column? What is a key?](#2-what-is-a-table-row-column-what-is-a-key)
- [3) The data story as an illustrative simple field-trial dataset](#3-the-data-story-as-an-illustrative-simple-field-trial-dataset)
- [4) Create the schema (PostgreSQL)](#4-create-the-schema-postgresql)
- [5) Insert a small sample dataset](#5-insert-a-small-sample-dataset)
- [6) SQL queries in increasing difficulty](#6-sql-queries-in-increasing-difficulty)
- [7) Quick recap of common mistakes](#7-quick-recap-of-common-mistakes)

In this post, we go from SQL fundamentals to a practical farm trial workflow. Focusing in understanding tables and keys, designing a clean schema, creating and loading sample data, running core SQL queries, checking data quality, and interpreting results with clear limits.
...
- `harvests.treatment_id -> treatments.treatment_id`
- `weather_daily.farm_id -> farms.farm_id`
...
Interesting interpretation note:
...
## 6.11 A practical interpretation check beginners often skip
...
- Never run `UPDATE` without `WHERE` unless you truly want to modify all rows.
...
- Always run the matching `SELECT` first to verify which rows will be deleted.
...
PostgreSQL vs Oracle note:

- PostgreSQL commonly uses `ON CONFLICT`.
