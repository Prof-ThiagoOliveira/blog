---
# Display name
title: Thiago de Paula Oliveira

# Name pronunciation (optional)
name_pronunciation: 

# Full name (for SEO)
first_name: Thiago
last_name: Oliveira

# Status emoji
status:
  icon: ""

# Is this the primary user of the site?
superuser: true

# Highlight the author in author lists? (true/false)
highlight_name: true

# Role/position/tagline
role: Statistician

# Organizations/Affiliations to display in Biography blox
organizations:
  - name: AbacusBio
    url: https://abacusbio.com/

# Social network links
# Need to use another icon? Simply download the SVG icon to your `assets/media/icons/` folder.
profiles:
  - icon: at-symbol
    url: 'mailto:thiago.oliveira@ed.ac.uk'
    label: E-mail Me
  - icon: brands/x
    url: https://twitter.com/_OliveiraTP_
  - icon: brands/github
    url: https://github.com/Prof-ThiagoOliveira
  - icon: academicons/google-scholar
    url: https://scholar.google.com.br/citations?hl=pt-BR&user=KKY2ynUAAAAJ
  - icon: academicons/orcid
    url: https://orcid.org/0000-0002-4555-2584

interests:
  - Statistics and biostatistics
  - Concordance analysis
  - Multilevel and forecast models
  - Generalized mixed-effects models
  - Longitudinal data
  - Agriculture
  - Sports (athlete performance)
  - Dashboard app development
  - Economic selection indexes

education:
  - area: PhD in Statistics
    institution: University of S├úo Paulo
    date_start: 2014-01-01
    date_end: 2018-12-31
    summary: |
      Dissertation on advanced statistical modelling in agricultural data, with a focus on longitudinal data analysis and multilevel models. Completed a research internship at the National University of Ireland Galway.
    button:
      text: 'Read Dissertation'
      url: 'https://www.teses.usp.br/teses/disponiveis/11/11134/tde-01082018-184101/publico/Thiago_de_Paula_Oliveira_versao_simplificada.pdf'

  - area: MSc in Statistics
    institution: University of S├úo Paulo
    date_start: 2012-01-01
    date_end: 2014-12-31
    summary: |
      Thesis on statistical methods for concordance analysis. Achieved a GPA of 3.9/4.0.

  - area: BSc in Agricultural Engineering
    institution: University of S├úo Paulo
    date_start: 2007-01-01
    date_end: 2012-12-31
    summary: |
      Specialized in agricultural systems and statistical applications in agriculture. Graduated with honors.

work:
  - position: Statistician
    company_name: AbacusBio
    company_url: https://abacusbio.com/
    company_logo: ''
    date_start: 2023-04-01
    date_end: ''
    summary: |
      - Develop advanced statistical models and economic selection indexes for breeding programs.
      - Design and implement data visualization dashboards and Docker containers.
      - Conduct quantitative genetics and genomics research for plant and animal breeding.
      - Collaborate with multidisciplinary teams to deliver impactful research solutions.
      - Provide statistical consulting and training to internal and external stakeholders.

  - position: Researcher fellow in quantitative genetics
    company_name: The Roslin Institute, University of Edinburgh
    company_url: https://www.ed.ac.uk/roslin
    company_logo: ''
    date_start: 2020-01-01
    date_end: 2023-03-31
    summary: |
      - Developed statistical models for quantitative genetics and genomics of plant breeding.
      - Conducted research on genetic improvement and trait selection in crops.
      - Published research findings in high-impact scientific journals.
      - Presented research at international conferences and seminars.
      - Collaborated with industry partners to translate research into practical applications.

  - position: Postdoctoral researcher in biostatistics
    company_name: Insight Centre for Data Analytics, NUI Galway
    company_url: https://www.insight-centre.org/
    company_logo: ''
    date_start: 2019-01-01
    date_end: 2020-12-31
    summary: |
      - Developed statistical methods for optimizing athlete performance.
      - Contributed to predictive modelling for COVID-19.
      - Implemented longitudinal data analysis and multilevel models.
      - Published research in peer-reviewed journals.
      - Collaborated with sports scientists and data analysts.

  - position: Lecturer in statistics
    company_name: University of S├úo Paulo
    company_url: https://www5.usp.br/
    company_logo: ''
    date_start: 2017-01-01
    date_end: 2019-12-31
    summary: |
      - Taught undergraduate and graduate courses in statistics.
      - Supervised student research projects and theses.
      - Developed course materials and curriculum.
      - Conducted research in experimental statistics and statistical modelling.
      - Published research in academic journals.

  - position: Biostatistician
    company_name: Orreco
    company_url: https://www.orreco.com/
    company_logo: ''
    date_start: 2019-01-01
    date_end: 2019-12-31
    summary: |
      - Developed statistical models for sports performance analytics.
      - Provided data analysis and statistical consulting services.
      - Collaborated with sports scientists to optimize athlete performance.

# Skills
# Add your own SVG icons to `assets/media/icons/`
skills:
  - name: Programming Languages
    items:
      - name: R
        description: Advanced proficiency
        percent: 96
        icon: devicon/rstudio
      - name: C++
        description: Intermediate proficiency
        percent: 75
        icon: devicon/cplusplus
      - name: MATLAB
        description: Intermediate proficiency
        percent: 70
        icon: devicon/matlab
      - name: Bash/Shell
        description: Advanced proficiency
        percent: 70
        icon: devicon/bash
      - name: Python
        description: Beginner proficiency
        percent: 25
        icon: devicon/python

  - name: Statistical Software
    items:
      - name: Shiny
        description: Advanced proficiency
        percent: 95
        icon: devicon/rstudio
      - name: Octave
        description: Beginner proficiency
        percent: 40
        icon: devicon/matlab
      - name: Maple
        description: Intermediate proficiency
        percent: 65
        icon: devicon/matlab
      - name: SageMath
        description: Intermediate proficiency
        percent: 60
        icon: devicon/python
      - name: Quarto/Markdown
        description: Advanced proficiency
        percent: 90
        icon: devicon/markdown
      - name: BLUPF90
        description: Advanced proficiency
        percent: 90
        icon: command-line
      - name: Fimpute
        description: Advanced proficiency
        percent: 85
        icon: command-line

  - name: Development Tools
    items:
      - name: Docker
        description: Advanced proficiency
        percent: 85
        icon: devicon/docker
      - name: Visual Studio
        description: Intermediate proficiency
        percent: 75
        icon: devicon/visualstudio
      - name: Jupyter
        description: Intermediate proficiency
        percent: 85
        icon: devicon/jupyter
      - name: GitHub
        description: Advanced proficiency
        percent: 85
        icon: brands/github

  - name: Platforms and Frameworks
    items:
      - name: LaTex
        description: Advanced proficiency
        percent: 90
        icon: devicon/latex
      - name: Office 365
        description: Advanced proficiency
        percent: 90
        icon: devicon/office
      - name: Linux
        description: Advanced proficiency
        percent: 80
        icon: devicon/linux

  - name: Hobbies
    color: '#eeac02'
    color_border: '#f0bf23'
    items:
      - name: Photography
        description: Semi-professional proficiency
        percent: 80
        icon: hero/camera
      - name: Hiking
        description: Enthusiast
        percent: 75
        icon: hero/map
      - name: Travel
        description: Avid traveler
        percent: 85
        icon: globe-alt
        
# Languages Section

languages:
  - name: Portuguese
    percent: 100

  - name: English
    percent: 85
    
  - name: Spanish
    percent: 30    

# Awards
# Add/remove as many awards below as you like.
# Only `title`, `awarder`, and `date` are required.
# Begin multi-line `summary` with YAML's `|` or `|2-` multi-line prefix and indent 2 spaces below.
awards:
  - title: Marie Sk┼éodowska-Curie Actions (MSCA) COFOUND Fellowship (Train@Ed)
    url: https://www.ed.ac.uk/roslin
    date: 2020-04-01
    awarder: The Roslin Institute, University of Edinburgh
    summary: |
      Awarded for research excellence in quantitative genetics and genomics.

  - title: Research Excellence Award
    date: 2014-01-01
    awarder: University of S├úo Paulo
    summary: |
      Recognized for outstanding contributions to statistical modelling and data analysis.

---

## About Me

<div style="text-align: justify;">
I am Thiago de Paula Oliveira, a statistician at AbacusBio. My work spans statistical modelling across agriculture, genetics, and sports analytics. I design and implement advanced mixedΓÇæmodel and Bayesian methods, develop economic and sustainability selection indices, and build reproducible, interactive dataΓÇævisualisation dashboards. I also develop productionΓÇæquality code in R, C++, Bash, and other languages, including package development and workflow automation, to ensure scalable, transparent analyses. My focus is on delivering decisionΓÇæready outputs that improve agricultural productivity, accelerate genetic gain, and enhance athletic performance.
</div>
